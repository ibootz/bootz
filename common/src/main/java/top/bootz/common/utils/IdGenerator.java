...
public class IdGenerator implements Configurable, IdentifierGenerator {
	...
	public synchronized Serializable generate(SessionImplementor session, Object obj) throws HibernateException {
		...
		String time = DateUtil.date2str(DateUtil.now(), BasePatternConstants.DATE_FORMAT_PATTERN_6);
		return StringUtil.joinWith("", perfix, time, buf.toString(), String.valueOf(seqNum));
	}
	...
	public void configure(Type type, Properties params, ServiceRegistry arg2) throws MappingException {
		...
		this.timestamp = DateUtil.date2str(DateUtil.now(), BasePatternConstants.DATE_FORMAT_PATTERN_4);
	...
	// int idLength = 24;
	// String perfix = "OSID";
	// String timestamp = DateUtil.date2str(new Date(),
	...
	// String time = DateUtil.date2str(DateUtil.now(),
